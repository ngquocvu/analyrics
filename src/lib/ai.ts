...
export const generateMeaning = async (songTitle: string, artist: string): Promise<AnalysisResult | null> => {
    ...
    Trả về dữ liệu dưới dạng JSON thuần túy (không có markdown formatting như \`\`\`json).
    
    Cấu trúc JSON:
    {
      "fullLyrics": "TOÀN BỘ lời bài hát đầy đủ, chính xác từ nguồn web. Giữ nguyên format, xuống dòng, và cấu trúc như trên web.",
      "vibe": "1 câu ngắn gọn về vibe/cảm xúc chủ đạo (ví dụ: Suy, Chữa lành, Flex...)",
      "overview": "Tóm tắt ngắn gọn nội dung bài hát (khoảng 2-3 câu).",
      "analysis": [
        { 
          "section": "Tên đoạn - PHẢI dùng format chuẩn: Intro, Verse 1, Verse 2, Pre-Chorus, Chorus, Post-Chorus, Bridge, Outro, hoặc Hook", 
          "lyricsQuote": "Nếu đoạn ngắn (dưới 4 câu): trích dẫn TOÀN BỘ lời của đoạn. Nếu đoạn dài: trích 3-4 câu QUAN TRỌNG NHẤT làm đại diện",
          "content": "Phân tích ý nghĩa đoạn này." 
        }
      ],
      "metaphors": [
        { "phrase": "Câu hát/Từ ngữ ẩn dụ đắt giá", "meaning": "Giải nghĩa slang/ẩn dụ đó." }
      ],
      "coreMessage": "Thông điệp cốt lõi ngắn gọn."
    }
    ...
    Giọng văn: Khách quan, sâu sắc, hiện đại, ngôn ngữ Gen Z Việt Nam.
    `;
